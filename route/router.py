...
from ubix.chain.chain_din_sql_sap import get_din_chain as get_din_chain_sap
from ubix.chain.chain_din_sql_bone import get_din_chain as get_din_chain_bone
from ubix.chain.chain_api_ubix import get_api_chain
...
def get_route_meta(text=None, schema_links=None):
    llm_type = os.environ.get("LLM_TYPE")
    route_meta = {
        "api": get_api_chain(llm),
        #"query": get_din_chain(llm) if llm_type == "din" else get_db_chain(llm),
        "query": [get_din_chain_sap(llm), get_din_chain_bone(llm)],
        "download": get_din_chain(llm) if llm_type == "din" else get_db_chain(llm),
